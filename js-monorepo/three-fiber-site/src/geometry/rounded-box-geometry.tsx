...
class RoundedBoxGeometry extends BoxGeometry {
  constructor(width = 1, height = 1, depth = 1, segments = 2, radius = 0.1) {
    ...
    const box = new Vector3(width, height, depth).divideScalar(2).subScalar(_radius);
    ...
    for (let i = 0, j = 0; i < positions.length; i += 3, j += 2) {
      ...
      switch (side) {
        case 0: // right
          ...
          uvs[j + 1] = 1.0 - getUv(faceDirVector, normal, 'y', 'z', _radius, height);
          break;

        case 1: // left
          faceDirVector.set(-1, 0, 0);
          uvs[j + 0] = 1.0 - getUv(faceDirVector, normal, 'z', 'y', _radius, depth);
          uvs[j + 1] = 1.0 - getUv(faceDirVector, normal, 'y', 'z', _radius, height);
          break;

        case 2: // top
          faceDirVector.set(0, 1, 0);
          uvs[j + 0] = 1.0 - getUv(faceDirVector, normal, 'x', 'z', _radius, width);
        ...
        case 3: // bottom
          ...
          faceDirVector.set(0, -1, 0);
          uvs[j + 0] = 1.0 - getUv(faceDirVector, normal, 'x', 'z', _radius, width);
          uvs[j + 1] = 1.0 - getUv(faceDirVector, normal, 'z', 'x', _radius, depth);
          break;

        case 4: // front
          // generate UVs along X then Y
          faceDirVector.set(0, 0, 1);
          uvs[j + 0] = 1.0 - getUv(faceDirVector, normal, 'x', 'y', _radius, width);
          uvs[j + 1] = 1.0 - getUv(faceDirVector, normal, 'y', 'x', _radius, height);
          break;
